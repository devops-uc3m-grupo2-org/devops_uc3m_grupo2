# Planificación del proceso de desarrollo — NewsRadar

## Calendario de sprints

| Sprint | Objetivo | Fechas |
|---|---|---|
| 0 | Inception + Infraestructura reproducible | 9-mar – 15-mar 2026 |
| 1 | Autenticación segura + gestión usuarios/roles | 16-mar – 22-mar 2026 |
| 2 | Gestión fuentes y canales RSS | 23-mar – 29-mar 2026 |
| 3 | Sistema de alertas CRUD + etiquetado | 30-mar – 5-abr 2026 |
| 4 | Integración intensiva IA generativa | 6-abr – 12-abr 2026 |
| 5 | Motor de monitorización RSS + notificaciones | 13-abr – 19-abr 2026 |
| 6 | Panel de mando + métricas + visualización | 20-abr – 26-abr 2026 |
| 7 | Calidad, CI/CD, documentación y defensa | 27-abr – 10-may 2026 |
| Buffer | Buffer + correcciones + verificación | 11-may – 24-may 2026 |
| Verificación final | Combate + clasificación + defensa | 25-may 2026 10:00 |

## Resultado por sprint

El detalle de lo implementado en cada sprint está en los sprint reviews:

- [Sprint 0 y 1](sprint_review_0_y_1.md)
- [Sprint 2](sprint_review_2.md)
- [Sprint 3](sprint_review_3.md)
- [Sprint 4](sprint_review_4.md)
- [Sprint 5](sprint_review_5.md)
- [Sprint 6](sprint_review_6.md)
- [Sprint 7](sprint_review_7.md)
